...
- [x] **Manejo de Sesión (JWT)**: Implementar la decodificación del token JWT recibido en `/login` para obtener el `usuario_id` y permisos.
- [x] **Marketplace de Colaboradores**: Crear una vista de "Marketplace" que consuma `/subcategorias/:id/colaboradores` para que el usuario pueda elegir un profesional antes de crear la solicitud. (Ahora con geolocalización dinámica).
- [ ] **Flujo de Solicitudes**: Actualizar la ruta `/pedir` para que esté vinculada a un `colaborador_id` real seleccionado del Marketplace. (En progreso).
...
- [x] **Consistencia de Estilos**: Asegurar que todas las vistas usen el mismo redondeo de botones (`btn-categoria`) y paleta de colores.
- [x] **Detalle de Trabajo Técnico**: Conectada la vista `detalle_trabajo_tecnico.html` desde el Dashboard.
